...
namespace GDMultiStash.Global
{
    ...
    internal partial class FileSystemManager : Base.Manager
    {
        public Utils.SimpleFileWatcher Watcher { get; private set; }

        public FileSystemManager() : base()
        {
        ...
        {
            if (!Directory.Exists(C.AppDataPath)) Directory.CreateDirectory(C.AppDataPath);
            if (!Directory.Exists(C.StashesPath)) Directory.CreateDirectory(C.StashesPath);
            if (!Directory.Exists(C.LocalesPath)) Directory.CreateDirectory(C.LocalesPath);
        }

        public string GetStashDirectory(int stashID)
        {
            return Path.Combine(C.StashesPath, stashID.ToString());
        }
        ...
        {
            int deletedFiles = 0;
            int maxBackups = G.Configuration.Settings.MaxBackups;
