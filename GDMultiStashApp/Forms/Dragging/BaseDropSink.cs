...
namespace GDMultiStash.Forms.Dragging
{
    ...
    internal abstract class BaseDropSink<T> : SimpleDropSink
    {

        protected BaseDragHandler<T> DragHandler { get; private set; }

        public List<T> OrderedList { get; private set; } = new List<T>();

        public int OverIndex { get; private set; } = -1;

        public BaseDropSink(BaseDragHandler<T> handler) : base()
        {
            DragHandler = handler;
