...
namespace IAGrim.Parser.Arc
{
    public class DDSImageReader
    {
        ...
        private static int[] BIT6 = {
            ...
            Console.WriteLine($"Extracting item icons from {itemsArcFullPath} to {destinationFolder}");

            try {
                if (!File.Exists(itemsArcFullPath)) {
                    Console.WriteLine($"The file {itemsArcFullPath} does not exist, icon extraction aborted.");
                }
                else if (!Directory.Exists(destinationFolder)) {
                    Console.WriteLine($"The specified output folder {destinationFolder} does not exist, icon extraction aborted.");
                }
                else {
                    ...
                    foreach (var icon in dc.strings) {
                        ...
                        if (b.Length > KiloByte30) {
                            Console.WriteLine($"Skipping {icon}, size is {b.Length / 1024}kb, max limit {KiloByte30 / 1024}kb");
                            continue;
                        }
                        ...
                        if (icon.EndsWith(".png")) {
                            ...
                            try {
                                ...
                                Console.WriteLine($"Error extracting icon \"{icon}\", {ex.Message}", ex);
                            }
                        }
                            ...
                            try {
                                ...
                                Console.WriteLine($"Parsing icon {icon}");
                ...
                Console.WriteLine("Tried to parse GD icons while GD was running: Access denied");
                throw;
            }
