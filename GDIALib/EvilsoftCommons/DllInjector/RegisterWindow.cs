    ...
    public class RegisterWindow : IDisposable
    {
        ...
        public class DataAndType
        {
            ...
            private string GetDataString(IntPtr ptr, int type)
            {
                ...
                {
                    Console.WriteLine("Error reading string from GD", ex);
                    Console.WriteLine($"This is possibly fatal, type: {type}");
                }
        ...
        public RegisterWindow(string className, Action<DataAndType> callback)
        {
            ...
            Console.WriteLine("[GDHook] Created window with hwnd " + m_hwnd);
            ...
            if (!ChangeWindowMessageFilterEx(m_hwnd, WM_COPYDATA, ChangeWindowMessageFilterExAction.Allow, IntPtr.Zero))
            {
                ...
                Console.WriteLine("[GDHook] Failed to remove UIPI filter restrictions.");
                if (isAdmin)
                    Console.WriteLine("[GDHook] Running as administrator, IA will not be able to communicate with GD due to UIPI filter.");
                else
                    Console.WriteLine("[GDHook] Not running as administrator, UIPI filter not required.");
            }
        }
    }
}
