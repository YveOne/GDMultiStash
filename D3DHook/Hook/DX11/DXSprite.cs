...
technique11 SpriteTech {
    pass P0 {
        ...
        public void DrawImage(DXImage image, int x, int y, int width, int height, System.Drawing.Color? color, float angle, float zIndex)
        {
            Debug.Assert(_initialized);
            ...
            using (var backupBlendState = _deviceContext.OutputMerger.GetBlendState(out SharpDX.Mathematics.Interop.RawColor4 backupBlendFactor, out int backupMask))
            {
                ...
                Draw(new Rectangle(x, y, width, height), new Rectangle(0, 0, image.Width, image.Height), color.HasValue ? ToColor4(color.Value) : Color4.White, 1.0f, angle, zIndex);
